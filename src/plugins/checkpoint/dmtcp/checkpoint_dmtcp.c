/*****************************************************************************\
 *  checkpoint_dmtcp.c - DMTCP slurm checkpoint plugin.
 *  $Id: checkpoint_dmtcp.c 0001 2008-12-29 16:50:11Z hjcao $
 *****************************************************************************
...
extern int slurm_ckpt_comp ( struct step_record * step_ptr, time_t event_time,
		uint32_t error_code, char *error_msg )
{
	error("checkpoint/dmtcp: slurm_ckpt_comp not implemented");
...
extern int slurm_ckpt_task_comp ( struct step_record * step_ptr,
				  ...
				  uint32_t error_code, char *error_msg )
{
	error("checkpoint/dmtcp: slurm_ckpt_task_comp not implemented");
	return ESLURM_NOT_SUPPORTED;
}
...
static void *_ckpt_agent_thr(void *arg)
{
	...
	if (rc != SLURM_SUCCESS) {
		error("checkpoint/dmtcp: error on checkpoint request %u to "
